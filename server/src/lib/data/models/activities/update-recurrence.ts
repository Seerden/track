...
import type { Recurrence, RecurrenceInput } from "@shared/types/data/recurrence.types";
...
export const updateRecurrence: QueryFunction<
	RecurrenceInput & { user_id: ID },
	Promise<Maybe<Recurrence>>
> = async ({ sql = sqlConnection, user_id, recurrence }) => {
	const { recurrence_id, user_id: _user_id, created_at, ...toUpdate } = recurrence;
